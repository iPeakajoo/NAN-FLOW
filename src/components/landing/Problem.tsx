...
export default function Problem() {
  ...
  return (
    <section id="problem" className="relative isolate overflow-hidden bg-[#F8FAFC] px-5 py-24 sm:px-6 lg:px-8">
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-12 grid gap-4 md:grid-cols-3">
          {t.problem.metrics.map((metric) => (
            <div
              key={metric.value}
              className="scroll-fade-up rounded-2xl bg-white/84 p-6 text-center shadow-lg ring-1 ring-gray-950/5 backdrop-blur transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
            >
              <p className="text-4xl font-bold tracking-tight text-[#0F766E] sm:text-5xl">
                {metric.value}
              </p>
              <p className="mx-auto mt-3 max-w-56 text-sm font-semibold leading-6 text-gray-600">
                {metric.label}
              </p>
            </div>
          ))}
        ...
        <div className="mt-6 grid gap-5 md:grid-cols-3">
          {t.problem.cards.map((card, index) => {
            ...
            return (
              <Card
                key={card.title}
                className="scroll-fade-up group flex h-full animate-in fade-in slide-in-from-bottom-3 flex-col rounded-xl border-gray-100 bg-white/90 shadow-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
              >
